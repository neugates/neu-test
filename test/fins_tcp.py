...
class FinsTcpRandom(HttpUser):
    wait_time = between(0.1, 2)

    def on_start(self):
        self.node_name = "fins_tcp"
        ...
        self.c.add_node(self.node_name, config.PLUGIN_FINS_TCP)
        self.c.add_group(self.node_name, self.group_name, 100)
        fins_node_setting(self.c, self.node_name, '192.168.10.102', 9600, 1)
    ...
    @task(10)
    def read_tags(self):
        request_name = "fins tcp random read tags"
        ctag.read_and_check(self.c,
                            request_name, self.node_name, self.group_name)

    @task(1)
    def write_tag(self):
        request_name = "fins tcp random write tag"
